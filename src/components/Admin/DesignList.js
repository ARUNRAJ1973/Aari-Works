...
const DesignList = () => {
  ...
  const handleSave = async (design) => {
    try {
      // Update the design
      await axios.put(`http://localhost:3001/designs/${design.id}`, design);

      // Fetch all carts
      const cartsResponse = await axios.get('http://localhost:3001/carts');
      ...
      for (const cart of allCarts) {
        ...
        if (JSON.stringify(updatedItems) !== JSON.stringify(cart.items)) {
          await axios.patch(`http://localhost:3001/carts/${cart.id}`, { items: updatedItems });
        }
      }
